...
# CORE FUNCTIONS ##############################################################
    def schedule_autopurge(self):
        ...
        self.timer.start()
        logger.debug("Automatic untouched tasks check scheduled")

    def cancel_autopurge(self):
        if self.timer:
            logger.debug("Automatic untouched tasks check cancelled")
            self.timer.cancel()
    ...
    def add_untouched_tag(self, widget=None):
        ...
        logger.debug("Starting process for adding %s", tag_name)
        ...
        for task in closed_tasks:
            ...
            if new_time < today:
                logger.debug('Adding %r tag to: %r as last time it was '
                             'modified was %r', tag_name, task.get_title(),
                             modified_time)
