...
def open_file(xml_path: str, root_tag: str) -> etree.ElementTree:
    ...
    for index, filepath in enumerate(files):
        try:
            ...
            logger.debug('File not found: %r. Trying next.', filepath)
            continue

        except PermissionError:
            logger.debug('Not allowed to open: %r. Trying next.', filepath)
            continue

        except etree.XMLSyntaxError as e:
            logger.debug('Syntax error in %r. %r. Trying next.', filepath, e)
            continue
...
def write_backups(filepath: str) -> None:
    ...
    try:
        ...
        logger.error('Backup dir %r cannot be created!', backup_dir)
        return
...
def create_dirs(filepath: str) -> None:
    ...
    try:
        ...
        logger.error("Error while creating directories: %r", error)
...
def save_file(filepath: str, root: etree.ElementTree) -> None:
    ...
    try:
        ...
        logger.error('Could not write XML file at %r', filepath)
