    ...
    def _load_pickled_file(self, path, default_value=None):
        ...
        with open(path, 'rb') as file:
            try:
                ...
                logger.error("Pickle file for backend '%s' is damaged",
                             self.get_name())
            ...
            if os.path.exists(backup_file):
                with open(backup_file, 'rb') as file:
                    try:
                        ...
                        logger.info("Succesfully restored backup #%d for %r",
                                    i, self.get_name())
                        return data
                    except Exception:
                        logger.error("Backup #%d for %r is damaged as well",
                                     i, self.get_name())

        # Data could not be loaded, degrade to default data
        logger.error("There is no suitable backup for %r, "
